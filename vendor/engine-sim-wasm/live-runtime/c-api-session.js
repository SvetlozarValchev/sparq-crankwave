...
export class EngineSimSession {
  ...
  enqueuePresentationControls(controls) {
    this.#assertAlive();
    if (!Array.isArray(controls) || controls.length === 0) {
      throw new TypeError("presentation controls must be a non-empty array");
    }
    if (controls.length > this.#descriptor.controlCommandQueueCapacity) {
      throw new EngineSimRuntimeError(
        "presentation-control batch exceeds the session queue capacity",
        {
          operation: "enqueue-presentation-controls",
          detailCode: "browser-runtime-presentation-control-capacity",
          diagnostics: [],
        },
      );
    }
    const pointer = this.#heap.allocate(
      controls.length * Layout.controlCommand.size,
      "live-presentation command batch",
    );
    const rejection = this.#heap.allocate(
      Layout.controlRejection.size,
      "live-presentation rejection",
    );
    try {
      const view = this.#heap.view;
      this.#heap.bytes.fill(
      ...
      for (let index = 0; index < controls.length; ++index) {
        const input = controls[index];
        const frame = normalizeU64(
          input.deliveryFrame,
          "presentation control deliveryFrame",
        );
        if (priorFrame !== null && frame < priorFrame) {
          throw new TypeError(
            "presentation controls must be ordered by deliveryFrame",
          );
        }
        priorFrame = frame;
        const base = pointer + index * Layout.controlCommand.size;
        ...
        let routeId = 0;
        switch (input.kind) {
          case "presentation-route-monitoring-gain":
            kind = ControlKind.presentationRouteMonitoringGain;
            value = requireNonnegative(input.value, "presentation route gain");
            routeId = input.routeId;
            break;
          case "presentation-route-wet-mix":
            kind = ControlKind.presentationRouteWetMix;
            value = requireUnitInterval(input.value, "presentation route wet mix");
            routeId = input.routeId;
            break;
          case "presentation-route-transfer-gain":
            kind = ControlKind.presentationRouteTransferGain;
            value = requireNonnegative(input.value, "presentation route transfer gain");
            routeId = input.routeId;
            break;
          case "presentation-master-monitoring-gain":
            kind = ControlKind.presentationMasterMonitoringGain;
            value = requirePositive(input.value, "presentation master gain");
            break;
          case "presentation-jitter-scale":
            kind = ControlKind.presentationJitterScale;
            value = requireNonnegative(input.value, "presentation jitter scale");
            break;
          case "presentation-jitter-cutoff":
            kind = ControlKind.presentationJitterCutoff;
            value = requirePositive(input.value, "presentation jitter cutoff");
            break;
          case "presentation-derivative-mix":
            kind = ControlKind.presentationDerivativeMix;
            value = requireUnitInterval(input.value, "presentation derivative mix");
            break;
          case "presentation-air-noise-mix":
            kind = ControlKind.presentationAirNoiseMix;
            value = requireUnitInterval(input.value, "presentation air-noise mix");
            break;
          case "presentation-air-noise-cutoff":
            kind = ControlKind.presentationAirNoiseCutoff;
            value = requirePositive(input.value, "presentation air-noise cutoff");
            break;
          case "presentation-cylinder-gain":
            kind = ControlKind.presentationCylinderGain;
            value = requireNonnegative(input.value, "presentation cylinder gain");
            routeId = input.cylinderOrdinal;
            break;
          case "simulation-throttle-gamma":
            kind = ControlKind.simulationThrottleGamma;
            value = requirePositive(input.value, "simulation throttle gamma");
            break;
          case "simulation-combustion-cycle-variation":
            kind = ControlKind.simulationCombustionCycleVariation;
            value = requireUnitInterval(input.value, "simulation cycle variation");
            break;
          case "simulation-rotating-inertia-scale":
            kind = ControlKind.simulationRotatingInertiaScale;
            value = requirePositive(input.value, "simulation rotating inertia scale");
            break;
          default:
            throw new TypeError(
              `unsupported presentation control: ${String(input.kind)}`,
            );
        }
        if (
          kind === ControlKind.presentationRouteMonitoringGain ||
          ...
          if (!Number.isSafeInteger(routeId) || routeId <= 0 || routeId > 0xffffffff) {
            throw new TypeError("presentation route ID must be a nonzero uint32");
          }
          view.setUint32(base + Layout.controlCommand.idValue, routeId, true);
        } else if (kind === ControlKind.presentationCylinderGain) {
          if (
            !Number.isSafeInteger(routeId) ||
            routeId <= 0 ||
            routeId > 0xffffffff
          ) {
            throw new TypeError("presentation cylinder ordinal must be a one-based uint32");
          }
          view.setUint32(base + Layout.controlCommand.idValue, routeId, true);
        }
        view.setUint32(base + Layout.controlCommand.kind, kind, true);
        view.setFloat64(base + Layout.controlCommand.scalarValue, value, true);
      }
      const status = this.#module._eso_session_enqueue_presentation_controls(
        this.#context,
        this.#handle,
        pointer,
        controls.length,
      ...
      this.#client.assertStatus(status, "enqueue-presentation-controls");
      return { accepted: controls.length };
    } finally {
      this.#heap.free(rejection);
      this.#heap.free(pointer);
