...
/** A small allocation-bounded SHA-256 implementation for SPARQ runtimes without SubtleCrypto. */
export function sha256(input: ArrayBuffer | ArrayBufferView): Uint8Array {
  const source = bytes(input);
  const state = new Uint32Array(INITIAL_STATE);
  const words = new Uint32Array(64);

  const process = (block: Uint8Array, offset: number): void => {
  ...
  const completeBytes = source.byteLength - (source.byteLength % 64);
  for (let offset = 0; offset < completeBytes; offset += 64) {
    process(source, offset);
  }

  const remainder = source.byteLength - completeBytes;
  const finalBytes = remainder < 56 ? 64 : 128;
  const final = new Uint8Array(finalBytes);
  final.set(source.subarray(completeBytes));
  final[remainder] = 0x80;
  const bitLengthHigh = Math.floor(source.byteLength / 0x20000000);
  const bitLengthLow = (source.byteLength * 8) >>> 0;
  const finalView = new DataView(final.buffer);
  finalView.setUint32(finalBytes - 8, bitLengthHigh, false);
  finalView.setUint32(finalBytes - 4, bitLengthLow, false);
  for (let offset = 0; offset < finalBytes; offset += 64) {
    process(final, offset);
  }
...
export function sha256Hex(input: ArrayBuffer | ArrayBufferView): string {
  return Array.from(sha256(input), (value) => value.toString(16).padStart(2, '0')).join('');
}

export const SHA256_CRYPTO = Object.freeze({
